...
describe('server', function () {
  ...
  describe('/auth', function() {
    it('valid register request, ec = 200', function(done) {
      http.get(httpRequest + '/auth/register?email=aaa@aaa.aaa&password=123', function (res) {
        assert.equal(200, res.statusCode);
        done();
      })
    });

    it('valid login request, ec = 200', function(done) {
      http.get(httpRequest + '/auth/login?email=aaa@aaa.aaa&password=123', function (res) {
        assert.equal(200, res.statusCode);
        done();
      })
    });

    it('register: missing password, return error code 400', function(done) {
      http.get(httpRequest + '/auth/register?email=bbb@bbb.bbb', function (res) {
